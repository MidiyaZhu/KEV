...
class Trainer(object):
    ...
    def train(self, data_loader):
        ...
        trainaccuracy = accuracy_score(emo_gold_array, emo_pred_array)
        f1 = f1_score(emo_gold_array, emo_pred_array, average='weighted')

        loss = np.mean(loss_array)

        return loss, f1,trainaccuracy
    ...
    def eval(self, data_loader):
        ...
        emo_gold_array = np.concatenate(emo_gold_array)
        emo_pred_array = np.concatenate(emo_pred_array)
        testaccuracy = accuracy_score(emo_gold_array, emo_pred_array)
        f1 = f1_score(emo_gold_array, emo_pred_array, average='weighted')
        loss = np.mean(loss_array)

        return loss, f1,testaccuracy
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        file=  r'/media/server01/sda2/zixiao/knowledgemapping/knowledgemapping/FTnewcode/result/hitrans_meld.txt'
        trainer.load("./checkpoint/model.pkl")
        testf1, testmacro, testmicro = [], [], []
        bestf1list, bestmalist, bestmilist = [], [], []
        testaccuracy, testaccuracy_l = [], []
        for i in range(20):
            dev_loss, dev_f1 ,dev_acc= trainer.eval(dev_loader)
            logger.info("Dev Loss: {:.4f} F1: {:.4f}".format(dev_loss, dev_f1))
            test_loss, test_f1 ,test_acc= trainer.eval(test_loader)
        ...
        for epoch in range(args.epochs):
            train_loss, train_f1,train_acc = trainer.train(train_loader)
